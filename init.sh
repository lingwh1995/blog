...
function generateChapterShardingsAndWriteFrontmatterForShardings() {
    ...
    do
        ...
        #给所有二级标题后面添加锚点使用的id
        sed -i 's/\(^## '"$i"'\.\([1-9][0-9]\?\).*\)/\1 \{#'"$i"'-\2\}/g' $2/$3/$1-chapter-$CHAPTER_NAME.md
        sed -i 's/\(^### '"$i"'\.\([1-9][0-9]\?\)\.\([1-9][0-9]\?\)\..*\)/\1 \{#'"$i"'-\2-\3\}/g' $2/$3/$1-chapter-$CHAPTER_NAME.md
        sed -i 's/\(^#### '"$i"'\.\([1-9][0-9]\?\)\.\([1-9][0-9]\?\)\.\([1-9][0-9]\?\)\..*\)/\1 \{#'"$i"'-\2-\3-\4\}/g' $2/$3/$1-chapter-$CHAPTER_NAME.md
        #写入锚点跳跃组件
        echo '' >> $2/$3/$1-chapter-$CHAPTER_NAME.md
