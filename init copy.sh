...
function cloneDocumentsOriginalFromRemote() {
    ...
    #获取插件启用状态
    PLUGIN_ENABLE_STATE=( $( parseIni ./config/bootstrap/plugins.ini clone-original-plugin enable) )
    ...
    then
        ...
        #获取远程文档原件仓库地址
        GIT_REPOSITORY_URL=( $( parseIni ./config/bootstrap/plugins.ini clone-original-plugin git-repository-url) )
        echo '远程文档原件仓库地址：'$GIT_REPOSITORY_URL
        
        echo '开始从远程文档原件仓库'$GIT_REPOSITORY_URL'拉取文档原件到本地...................'
        git clone $GIT_REPOSITORY_URL
        echo '完成从远程文档原件仓库'$GIT_REPOSITORY_URL'拉取文档原件到本地...................'
        
        echo '开始清理本地文档原件仓库'$LOCAL_DOCUMENTS_ORIGINAL_REPOSIROTY_NAME'文件夹中无用文件...................'
        #删除多余的.git文件
        rm -rf books/.git
        #删除多余的jenkinsfile文件
        ...
        #删除config文件夹
        rm -rf books/config
        echo '完成清理本地文档原件仓库'$LOCAL_DOCUMENTS_ORIGINAL_REPOSIROTY_NAME'文件夹中无用文件...................'
    fi
...
:<< EOF
    执行增强操作
    门面模式，将preExecuteEnhance、executeEnhance、afterExecuteEnhance三个方法封装到一起，一次性调用
EOF
function enhance() {
    ...
    do
        ...
        then
            ...
            #增强之前的前置操作
            preExecuteEnhance $MD_FILE_NAME $MD_FILE_RELATIVE_PATH

            #进行增强处理
            executeEnhance $MD_FILE_NAME $MD_FILE_RELATIVE_PATH $INCLUDE_CODE_PROJECT_NAME

            #增强之后的后置操作
            afterExecuteEnhance $MD_FILE_NAME $MD_FILE_RELATIVE_PATH
        fi    
    done
}
