...
:<< EOF
        #行号数字第一部分：固定值，这个值是1
        PATHS_LINENUMBER_STR_PART1=""
        #处理最外层情况
        if [ $i == 0 ]
        then
            PATHS_LINENUMBER_STR_PART1="1 "
        fi
        
        #行号数字第二部分：查询获取
        PATHS_LINENUMBER_STR_PART2=`ls -l $CURRENT_REALTIVE_PATH -R | grep -n 'docs/blogs/environment/centos.*' | cut -d: -f1 | sed 's/ /,/g' | tr '\r\n' ' ' `
        PATHS_LINENUMBER_STR=$PATHS_LINENUMBER_STR_PART1$PATHS_LINENUMBER_STR_PART2
...
EOF
        ...
        echo '=============================================YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY================'
        ...
        echo $IGNORE_FOLDER_LIST_STR
       
        echo '=============================================YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY================'

        #行号数组
        PATHS_LINENUMBER_STR=`ls -lR $CURRENT_REALTIVE_PATH -I README*.md $IGNORE_FOLDER_LIST_STR | grep -n 'docs/blogs/'  | cut -d: -f1 | sed 's/ /,/g' | tr '\r\n' ' ' `

        echo '行号数组字符串：'$PATHS_LINENUMBER_STR
        PATHS_LINENUMBER_ARR=( $PATHS_LINENUMBER_STR )

        #生成的内容中间先输出到这个文件中
        CACHE1_README_FILE_NAME="README-cache1.md"
        CACHE2_README_FILE_NAME="README-cache2.md"
        FINAL_README_FILE_NAME="README.md"

        #开始创建README.md文件的分片文件
        CACHE1_README_FILE_NAME_FULL_PATH_NAME=$CURRENT_REALTIVE_PATH/$CACHE1_README_FILE_NAME
        CACHE2_README_FILE_NAME_FULL_PATH_NAME=$CURRENT_REALTIVE_PATH/$CACHE2_README_FILE_NAME
        ...
        #rm -rf $CACHE1_README_FILE_NAME_FULL_PATH_NAME
        ...
        for(( j=0;j<${#PATHS_LINENUMBER_ARR[@]};j++)) do
            ...
            ls -lR $CURRENT_REALTIVE_PATH -I "README*.md*" $IGNORE_FOLDER_LIST_STR | \
            sed -n ''"$START_LINENUMBER"','"$END_LINENUMBER"'p' >> $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            ...
            #执行替换文件名前面部分的操作
            sed -i 's/^.*[0-9]\{2\}:[0-9]\{2\}/'"$NBSP"'-/g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            ...
            sed -i 's#\('"$NBSP"'-\) \(.*\)#\1 <a href="'"$HREF_STR"'\2">\2<\/a>#g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            
            #删除所有包含total和docs的的行
            sed -i '/\(^total\|^docs\)/d' $CACHE1_README_FILE_NAME_FULL_PATH_NAME

            #替换文件后缀名.md为空
            sed -i 's/.md</</g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME

            #将所有的.md替换为.html
            sed -i 's/\.md"/\.html"/g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME

            CHAPTER_FOLDER_NAME_CN=( $( parseIni ./config/i18n.ini breadcrumb $3) )
            #配置导航页面中的shardings（存放章节分片的文件夹）为中文
            #替换路径导航中的英文为中文    
            sed -i 's/.*- '"$3"'/'"$CHAPTER_FOLDER_NAME_CN"'/g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            #替换页面中的英文为中文
            sed -i 's/>'"$3"'</>'"$CHAPTER_FOLDER_NAME_CN"'</g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            ...
            #替换文件名称为文件内容合集
            sed -i 's/'"$1"'.html">'"$1"'</'"$1"'.html">'"$ALL_CHAPTERS_COLLECT_CN"'</g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME

            #替换章节0名称格式为只包含文件信息，如   centos7-chapter-1.安装Linux操作系统.md -> 1.安装Linux操作系统.md
            sed -i 's/>'"$1"'-chapter-0\.\(.*\)</>\1</g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
            #替换章节1到最后一章名称格式为只包含文件信息，如   centos7-chapter-1.安装Linux操作系统.md -> 1.安装Linux操作系统.md
            sed -i 's/>'"$1"'-chapter-\([0-9]\{1,2\}.*\)</>\1</g' $CACHE1_README_FILE_NAME_FULL_PATH_NAME
        ...
        cat  $CACHE1_README_FILE_NAME_FULL_PATH_NAME | grep - >> $FINAL_README_FILE_NAME_FULL_PATH_NAME
