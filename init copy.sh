...
#为xxx.md文件生成二级和三级目录大纲,并将这个目录大纲装换为markmap文件
...
###	简单版博客内容大纲
<Markmap localtion="/markmap/$/$1-outline2.html"/>

###	详细版博客内容大纲
<Markmap localtion="/markmap/$4/$1-outline3.html"/>
EOF
...
function createChapterOutLineGuidanceMd(){
    ...
    cd $2
    #获取一级标题总数
    TOTAL_TITLE1_COUNTS=`grep '^#\{1\} [1-9][0-9]\?' $1.md | tail -1 | cut -c 3-4 | sed 's/\.//g'`
...
<Markmap localtion="/markmap/$4/chapter/$1-outline5-chapter$i.html"/>
EOF
...
function createChapterMd(){
    ...
    do
        #获取章节起始行数
        CHAPTER_START_LINE_NUMBER=`grep -n '^# '"$i"'\.' $1.md | cut -d ':' -f 1`
        echo '章节'$i'起始行数：'$CHAPTER_START_LINE_NUMBER
        
        #获取章节结束行数
        CHAPTER_END_LINE_NUMBER=`grep -n '^# '"$[$i+1]"'\.' $1.md | cut -d ':' -f 1`
        CHAPTER_END_LINE_NUMBER=$[$CHAPTER_END_LINE_NUMBER-1]
        #如果计算出来的值为-1，说明已经扫描到了最后一行，单独处理一下最后一行的行数
        if [ $CHAPTER_END_LINE_NUMBER -eq "-1" ]
        then
            CHAPTER_END_LINE_NUMBER=`cat $1.md |wc -l`
        fi
        echo '章节'$i'结束行数：'$CHAPTER_END_LINE_NUMBER
        
        #获取章节名称
        CHAPTER_NAME=`grep -n '^# '"$i"'\.' $1.md | cut -d ' ' -f 2`
        echo '章节名称：'$CHAPTER_NAME

        #根据行号将内容输出到文件
        sed -n ''"$CHAPTER_START_LINE_NUMBER"','"$CHAPTER_END_LINE_NUMBER"'p' $1.md > chapters/$1-chapter-$CHAPTER_NAME.md
...
function createChapterNavListMd(){
    ...
    CHAPTER_NAV_LIST_MDFILE_NAME="$1-nav.md"
    #每次写入之前先删除旧文件，再写入新的内容
    rm -rf $CHAPTER_NAV_LIST_MDFILE_NAME
    echo " " >> $CHAPTER_NAV_LIST_MDFILE_NAME
    echo ">" >> $CHAPTER_NAV_LIST_MDFILE_NAME
    echo "# 分章节阅读" >> $CHAPTER_NAV_LIST_MDFILE_NAME

    #拼接博客介绍
    echo "- [x] 在Centos上搭建开发环境-<a href='./chapters/$1-chapter-0.博客介绍.html' target='_blank'>博客介绍</a>" >> $CHAPTER_NAV_LIST_MDFILE_NAME
    ...
    do
        ...
        #拼接真正导航列表内容
        echo "- [x] 在Centos上搭建开发环境-<a href='./chapters/$1-chapter-$CHAPTER_NAME.html' target='_blank'>$CHAPTER_NAME</a>" >> $CHAPTER_NAV_LIST_MDFILE_NAME
    done
    echo ">" >> $CHAPTER_NAV_LIST_MDFILE_NAME
...
function createSidebarConfigForMDAndMdChapters(){
    ...
    SIDEBAR_CONFIGFILE_NAME=$1".json"
    #每次写入之前先删除旧文件，再写入新的内容
    rm -rf $    SIDEBAR_CONFIGFILE_NAME=$1".json"
    echo "    {" >> $SIDEBAR_CONFIGFILE_NAME
    echo "      // 必要的，分组的标题文字" >> $SIDEBAR_CONFIGFILE_NAME
    echo "      text: \"在Centos7上搭建开发环境\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "      // 可选的, 分组标题对应的图标" >> $SIDEBAR_CONFIGFILE_NAME
    echo "      icon: \"a-archivecatalogue-fill\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "      // 可选的, 设置分组是否可以折叠，默认值是 false" >> $SIDEBAR_CONFIGFILE_NAME
    echo "      collapsable: true," >> $SIDEBAR_CONFIGFILE_NAME
    echo "      // 必要的，分组的子项目" >> $SIDEBAR_CONFIGFILE_NAME
    echo "      children: [" >> $SIDEBAR_CONFIGFILE_NAME
    
    #左侧导航栏拼接博客内容介绍

    #拼接一次阅读全部内容
    echo "        {" >> $SIDEBAR_CONFIGFILE_NAME
    echo "          text: \"博客内容介绍\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "          link:\"$2/chapters/$1-chapter-0.$CHAPTER_NAME.md\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "          icon:\"folder\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "        }," >> $SIDEBAR_CONFIGFILE_NAME
    ...
    do
        ...
        echo "          text:\"$CHAPTER_NAME\"," >> $SIDEBAR_CONFIGFILE_NAME
        echo "          link:\"$2/chapters/$1-chapter-$CHAPTER_NAME.md\"," >> $SIDEBAR_CONFIGFILE_NAME
        echo "          icon:\"note\"," >> $SIDEBAR_CONFIGFILE_NAME
    ...
    #拼接一次阅读全部内容
    echo "        {" >> $SIDEBAR_CONFIGFILE_NAME
    echo "          text: \"$[$TOTAL_TITLE1_COUNTS+1].一次阅读全部内容\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "          link: \"$2/$1.md\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "          icon:\"folder\"," >> $SIDEBAR_CONFIGFILE_NAME
    echo "        }" >> $SIDEBAR_CONFIGFILE_NAME
...
MD_FILE_CHAPTER_TARGET_PATH="docs/.vuepress/public/chapter/environment/centos"
createChapterMd $MD_FILE_NAME $MD_FILE_SOURCE_PATH $MD_FILE_CHAPTER_TARGET_PATH

#为md文件创建章节导航目录
createChapterNavListMd $MD_FILE_NAME $MD_FILE_SOURCE_PATH $MD_FILE_CHAPTER_TARGET_PATH

SIDEBAR_LINK_PREFIX="/blogs/environment/centos"

#为md文件和md文件拆分的所有章节md文件生成侧边栏配置，存放在json文件中
createSidebarConfigForMDAndMdChapters $MD_FILE_NAME $SIDEBAR_LINK_PREFIX
